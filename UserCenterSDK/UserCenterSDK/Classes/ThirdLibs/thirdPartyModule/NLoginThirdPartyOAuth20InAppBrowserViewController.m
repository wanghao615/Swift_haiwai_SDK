...
- (void)makeBottomBar   {
    ...
    [_closeBt setImage:[UIImage imageNamed:Bundle(@"login_naverClose")] forState:UIControlStateNormal];
...
- (void)makeBannerView
{
    ...
    naverAppIconView.image = [UIImage imageNamed:@"SDKImageResource.bundle/login_naverapp_logo"];
    ...
    bannerMessage.text = Localized(@"NaverAuthBannerMessage") ;
    ...
    NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:Localized(@"NaverAuthBannerDownloadLinkMessage")];
    ...
    UIImageView *closeImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"SDKImageResource.bundle/login_naverClose"]];
    closeImageView.backgroundColor = [UIColor redColor];
    _bannerCloseButton = [UIButton buttonWithType:UIButtonTypeCustom];
    closeImageView.frame = CGRectMake([[UIScreen mainScreen] bounds].size.width - 10 - 11, 10, 11, 11);
...
{
    _webView.navigationDelegate = aDelegate;
}
